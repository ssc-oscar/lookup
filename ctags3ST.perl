...
open CNT, "blob_ST_$sec.bin" or die "$!";
...
open A, "blob_ST_$sec.idx";
my ($n, $off, @rest) = split (/;/, <A>, -1);
my $offset = 0;
my $record = 0;
my $offsetStart = $off;
my $countStart = $n;
...
my $nn = -1;
open IDX, "zcat blob_ST_${sec}.idxf2|";
...
while (<IDX>){
  ...
  $i++;
  $nn ++;
  print STDERR "missed record at nn=$nn j=$j ".($j-$countStart)." off=$off len=$len b=$b\n" if ($nn+0 != $j-$countStart);
  $nn = $j-$countStart;
  next if $j < $from+$countStart; 
  last if ($to >= 0 && $j >=$to+$countStart);
  if ($f ne ""){
    $f =~ s|^.*/||;
    $f =~ s|[\s\[\]\{\}\(\)\!\?]|_|g;
  }
